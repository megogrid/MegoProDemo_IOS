...
@interface ViewController ()<UIImagePickerControllerDelegate,UINavigationControllerDelegate,MegoGridInAppModelDelegate>
{
...
//     UITapGestureRecognizer * tap=[[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(swipeRightGesture:)];
//    tap.s
...
-(void)loadImageView : (UIImage*)Image
{
    ...
    UIButton *saveButton=[[UIButton alloc]initWithFrame:CGRectMake(self.view.frame.size.width-50, 15, 30, 30)];
...
-(void)MewoInAppPro:(NSString*)boxId boxId:(BOOL)isprompt{
...
-(void)gotoShopCallback:(NSString *)featureId{


    CustumClass*custumClass=[[CustumClass alloc]init];
    [self.navigationController pushViewController:custumClass animated:YES];
}
...
-(void)CameraImageViewClick
{
    ...
    NSString *boxid;
    NSString *getApp=[[NSUserDefaults standardUserDefaults] valueForKey:@"getApp"];
    
    if ([getApp intValue]==12) {
        boxid=@"IHA9BOY5W";
    }
    else if ([getApp intValue]==13) {
        boxid=@"THY583ULZ";
    }
    else
    {
    ...
    [self MewoInAppPro:boxid boxId:TRUE];
...
-(void)PushToViewController:(long)indexPath
{
    switch(indexPath)
    {
        ...
        case 4:
        {
            DownloadImagesHistoryViewController *HistoryViewControllerObj=[[DownloadImagesHistoryViewController alloc]init];
            [self.navigationController pushViewController:HistoryViewControllerObj animated:YES];
...
#pragma mark MeUser Response Handler
